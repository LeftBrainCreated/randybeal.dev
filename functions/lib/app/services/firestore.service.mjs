...
let FirestoreService = class FirestoreService {
    ...
    async createUser(userProfile) {
        ...
        await faithfulDbConfig.collection("users").doc(userId).set(Object.assign(Object.assign({ userId: userId, userCreatedDate: userCreatedDate }, userProfile), { interactionLog: [] }), { merge: true });
        console.log("User created/updated successfully:", userProfile.id);
        console.log("User data:", userProfile);
        return userProfile.id; // Return the Firestore document ID
    }
    async updateUser(userId, userProfile) {
        ...
        // }
        await docRef.set({ profile: userProfile.profile }, { merge: true });
        return "done";
    }
    ...
    async getUserInteractions(userId, fromDate = new Date('Jan 1, 1900'), toDate = new Date()) {
    ...
    async saveUserStudy(userId, studyName, studyContent) {
...
//# sourceMappingURL=firestore.service.mjs.map